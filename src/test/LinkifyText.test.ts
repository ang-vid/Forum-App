...
describe("getYoutubeVideoId", () => {
	...
	it("ignores extra params like &t=", () => {
		expect(
			getYoutubeVideoId("https://www.youtube.com/watch?v=jNQXAC9IVRw&t=12s"),
		).toBe("jNQXAC9IVRw");
	});
});
...
describe("getTiktokVideoId", () => {
	...
	it("ignores extra params like ?lang=en", () => {
		...
		).toBe("7295454326111915306");
	});
});

describe("getInstagramPostId", () => {
	it("extracts id from /p/ url", () => {
		expect(getInstagramPostId("https://www.instagram.com/p/CuE2WNQs6vH/")).toBe(
			"CuE2WNQs6vH",
		);
	});
	...
	it("extracts id from /reel/ url", () => {
		expect(
			getInstagramPostId("https://www.instagram.com/reel/C5q0QK8O8Yw/"),
		).toBe("C5q0QK8O8Yw");
	});

	it("extracts id from /reels/ url", () => {
		expect(
			getInstagramPostId("https://www.instagram.com/reels/CuE2WNQs6vH/"),
		).toBe("CuE2WNQs6vH");
	});

	it("extracts id from /tv/ url", () => {
		expect(
			getInstagramPostId("https://www.instagram.com/tv/CXx1234abcD/"),
		).toBe("CXx1234abcD");
	});
});
...
describe("getSegmentType", () => {
	it("detects youtube.com", () => {
		expect(getSegmentType("https://www.youtube.com/watch?v=abc")).toBe(
			"youtube",
		);
	});
	...
	it("detects tiktok", () => {
		expect(getSegmentType("https://www.tiktok.com/@user/video/123")).toBe(
			"tiktok",
		);
	});

	it("detects instagram", () => {
		expect(getSegmentType("https://www.instagram.com/p/abc/")).toBe(
			"instagram",
		);
	});
