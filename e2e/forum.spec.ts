...
test('View button navigates to the correct post detail page', async ({ page }) => {
  await page.getByPlaceholder('Write your post...').fill('qwerty');
  await page.getByRole('button', { name: 'SEND' }).click();
...
test('can delete a post and it does not come back on refresh', async ({ page }) => {
...
test('cannot submit a post that is over the character limit', async ({ page }) => {
  await page.getByPlaceholder('Write your post...').fill('a'.repeat(1001));
  await expect(page.getByRole('button', { name: 'SEND' })).toBeDisabled();
});

test('link in post text is linkified and visible', async ({ page }) => {
  await page.getByPlaceholder('Write your post...').fill('werty https://example.com qewruuykuyt');
  await page.getByRole('button', { name: 'SEND' }).click();
